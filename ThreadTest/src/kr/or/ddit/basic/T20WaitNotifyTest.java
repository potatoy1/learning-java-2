...
class DataBox {
	...
	public synchronized void setData(String data) {
		if(this.data != null) {
			...
			System.out.println(Thread.currentThread().getName() + " noitfy() 호출");
			notify();
		}
	}
...
class ProducerThread extends Thread{
	...
	public void run() {
		for(int i = 1; i<=5; i++) {
			...
			dataBox.setData(data);							// 생산자스레드=> setData만 실행(이미 데이터가 있을 경우 wait()상태됨)
		}
	}
}
...
class ConsumerThread extends Thread{
	...
	public void run() {
		for(int i = 1; i<=5; i++) {
			String data = dataBox.getData();				// 소비자스레드=> getData만 실행(꺼내올 데이터가 없을 경우 wait()상태됨)
