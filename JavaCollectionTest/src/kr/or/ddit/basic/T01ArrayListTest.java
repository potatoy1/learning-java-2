...
public class T01ArrayListTest {
	public static void main(String[] args) {
		...
		list1.add("aaa");
		list1.add("bbb");
		list1.add(111);					//list.add(new Integer(111));과 같은것
		list1.add('k');					//자바의 기본타입은 다 오브젝트화 가능-> 기본타입의 값을 객체로 감싸놓음->래퍼클래스/오토박싱이 일어났다.
		list1.add(true);
		list1.add(12.34);
		...
		list1.remove(new Integer(111));					//list1.remove(111);은 인덱스 111번째 값을 지우려는것이므로 object로 선언해서 지워야함.
		...
		// indexOf(비교객체); => 리스트에서 '비교객체'를 찾아 '비교객체'가 있는 index값을 반환한다. 없으면 -1반환함.
